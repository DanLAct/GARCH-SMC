function [theta, theta_particle, loglike, logprior, W, log_evidence] = GJR_SMC_RW_DataAnneal_parallel(N)
%SMC utilised vectorisation and parallelisation for estimating GJR-GARCH(1,1) model's parameters;
...
% loglike            - Log likelihood of the GJR-GARCH(1,1) model, corresponding to the above thetas
% logprior           - Log prior of the GJR-GARCH(1,1) model, corresponding to the above thetas
...
%Initialising
Num_Parameter=5; % dimension of theta
...
logprior = GJR_logPrior_parallel_unif(theta_particle,mylims);
...
while isempty(inds)==0 %this while loop is used to eliminated any infinity value from simulation
    ...
    logprior(inds) = GJR_logPrior_parallel_unif(theta_particle(inds,:),mylims);
...
for t=1:T
    ...
    if t==1
        [loglike(:,t),h_gjr] = gjrloglike_parallel_unif(theta_particle(:,:,t),rate_return(1),mylims);
        logw(:,t+1) = log(W(:,t)) + loglike(:,t);
    else
        %log(f(y_t|y_{1:t-1},\theta))
        [loglike_inc,h_gjr] = gjrloglike_danneal_in_parallel(theta_particle(:,:,t),rate_return(t),mylims,h_gjr,rate_return(t-1));
    ...
    if ESS(t)<propotion*N %Checking if there's a need to resample
        ...
        logprior_prop = GJR_logPrior_parallel_unif(theta_particle_prop,mylims);
        ...
        parfor core = 1:poolsize
            ...
            [loglike_prop_cell{core},h_gjr_cell{core}] = gjrloglike_parallel_unif(theta_particle_prop(inds(current_core),:),rate_return_sub,mylims);
        end
        ...
        while belowThreshold
            ...
            parfor core = 1:poolsize
                current_core = percore*(core-1)+1: min(percore*core,len_inds);
                [loglike_prop_cell{core},h_gjr_prop_cell{core}] = gjrloglike_parallel_unif(theta_particle_prop(inds(current_core),:),rate_return_sub,mylims);
            end
            loglike_prop = loglike_prop_cell{1};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [logprior] = GJR_logPrior_parallel_unif(theta_particle,mylims)
% Computes the log prior (takes transformed parameters as input)

sumA =  [-eye(5); eye(5); 0 1 0.5 1 0];   
sumB =  [-1e-9; -1e-8; -1e-2; -.5; .9; 0.3; 0.3; 0.3; 0.99; 0.9; 0.9999];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [loglikelihood,h] = gjrloglike_danneal_in_parallel(params,new_data,mylims,h,data_previous)
% Computing the likelihood of the time series under GJR-GARCH(1,1) dynamics, given previous and new observed data and model parameters
%====================================================================================================================
...
if isempty(inds_neg)==0
    h_t(inds_neg) = h0(inds_neg) + rho_h(inds_neg).*h(inds_neg,end)+ phi_p(inds_neg).*(data_previous-r_bar(inds_neg)).^2 + phi_n(inds_neg).*(data_previous-r_bar(inds_neg)).^2;
end

if isempty(inds_pos)==0
    h_t(inds_pos) = h0(inds_pos) + rho_h(inds_pos).*h(inds_pos,end)+ phi_p(inds_pos).*(data_previous-r_bar(inds_pos)).^2;
end

obs=new_data-r_bar;
loglikelihood =- .5*log(h_t) - .5*(obs).^2./h_t -.5*log(2*pi*ones(N,1));
h = [h h_t];
