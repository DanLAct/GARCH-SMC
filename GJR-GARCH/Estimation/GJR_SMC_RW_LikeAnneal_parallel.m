function [theta, theta_particle, loglike, logprior, gamma, log_evidence] = GJR_SMC_RW_LikeAnneal_parallel(N)
%SMC utilised vectorisation and parallelisation for estimating GJR-GARCH(1,1) model's parameters; 
%The sequence of distributions are constructed by using Likelihood Annealing method
...
[loglike,~] = gjrloglike_parallel_unif(theta_particle,rate_return,mylims);
...
while gamma(t) < 1
    ...
    %Subsitute the value of just calculated gamma(t+1)
    ...
    % (systematic reseampling) - duplicating particles with more weights
    u = rand;
    indices = zeros(N,1);
    cumsum_w = cumsum(w);
    for i=1:N
    ...
    while isempty(inds)==0 %this while loop is used to eliminated any infinity value from simulation
        ...
        logprior_prop(inds) = GJR_logPrior_parallel_unif(theta_particle_prop(inds,:),mylims);
    ...
    parfor core = 1:poolsize %Divide Sample into different 'poolsize' groups
        ...
        [loglike_prop_cell{core},~] = gjrloglike_parallel_unif(theta_particle_prop(current_core,:),rate_return,mylims);
    end
    ...
    while belowThreshold
        ...
        while isempty(inds)==0 %this while loop is used to eliminated any infinity value from simulation
            ...
            logprior_prop(inds) = GJR_logPrior_parallel_unif(theta_particle_prop(inds,:),mylims);
            count_logprior(inds) = count_logprior(inds) + ones(length(inds),1);
            inds = find(isinf(logprior_prop));
        end
...
save('results_GJR_LikeAnneal.mat')
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
