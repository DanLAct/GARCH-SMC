function [theta, theta_particles, loglike, logprior, W, log_evidence] = SMC_RW_DataAnneal_Par_unif_unbiased_is(N)
...
%The likelihood is unbiasedly estimated using importance sampling
%
%%%%%%%%%%%%%%%%%%%% INPUT %%%%%%%%%%%%%%%%%%%%
...
for t=1:T
    ...
    if t==1
        parfor i=1:N
            [~, loglike(i,t),ptseries{i},ntseries{i},~] = bege_gjrgarch_likelihood_unbiased_is(theta_particles(i,:,t),rate_return(1),mylims);
        end
        logW(:,t+1) = log(W(:,t)) + loglike(:,t);
    else
        parfor i=1:N
            [loglike_inc(i),ptseries{i},ntseries{i}] = bege_gjrgarch_likelihood_danneal_in_unbiased_is(theta_particles(i,:,t),rate_return(t),mylims,ptseries{i},ntseries{i},rate_return(t-1));
        end
    ...
    if ESS(t)<propotion*N %Checking if there's a need to resample
        ...
        parfor i=1:N
            ...
            [~, loglike_prop,ptseries_prop,ntseries_prop,~] = bege_gjrgarch_likelihood_unbiased_is(theta_particles_prop, rate_return_sub,mylims);
...
%save('results_DataAnneal_unbiased_is.mat');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [loglikelihood,ptseries,ntseries] = bege_gjrgarch_likelihood_danneal_in_unbiased_is(params,new_data,mylims,ptseries,ntseries,data_previous)
...
obs = new_data -r_bar;
loglikelihood = loglikedgam_unbiased_is(obs,p_t,n_t,tp,tn);
